...
PairCoulSmooth::PairCoulSmooth(LAMMPS *lmp) : PairCoulCut(lmp) {}
...
void PairCoulSmooth::compute(int eflag, int vflag)
{
  ...
  double rsq,r2inv,r,rinv,forcecoul,factor_coul,ksmooth,rcut,rcutinv,energy;
...
/*modified*/
        ...
        energy = forcecoul;
        rcut = cut_global;
        rcutinv = 1/cut_global;
        ksmooth = 1 -(r*rcutinv) +r*(r-rcut)*rcutinv*rcutinv;
        forcecoul = forcecoul*ksmooth;
        fpair = factor_coul* energy* (r2inv-rcutinv*rcutinv);
...
double PairCoulSmooth::single(int i, int j, int itype, int jtype,
                           ...
                           double &fforce)
{
  double r2inv,r,rinv,forcecoul,phicoul,rcut,rcutinv,ksmooth,energy;
  ...
  ksmooth = 1 -(r*rcutinv) +r*(r-rcut)*rcutinv*rcutinv;
  forcecoul = forcecoul*ksmooth;
  fforce = factor_coul*energy * (r2inv-rcutinv*rcutinv);

  phicoul = force->qqrd2e * atom->q[i]*atom->q[j] * rinv * ksmooth;
