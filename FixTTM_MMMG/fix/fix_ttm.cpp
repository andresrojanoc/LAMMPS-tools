  ...
  net_energy_transfer(nullptr), net_energy_transfer_all(nullptr)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"set") == 0) {
      ...
      if (iarg+1 > narg) error->all(FLERR,"Illegal fix ttm command");
      cutoff_active = true;
      iarg += 1;
  ...
  if (cetable_active){
    for (int i = 0; i < Ce_values.size(); i++) {            
      if (Ce_values[i] <= 0.0)
        error->all(FLERR,"Fix ttm all electronic_specific_heat in the file must be > 0.0");
    };
  }
...
void FixTTM::TableInterpReader(const std::string &table_filename, const std::string &table_parametername)
{
...
double FixTTM::LinearInterpolate(const double &Temp_Ce, const std::string &table_parametername) {
