...
#include "pair_tersoff_mod_zbl.h"
...
PairTersoffMODZBL::PairTersoffMODZBL(LAMMPS *lmp) : PairTersoffMOD(lmp)
{
  ...
  if (strcmp(update->unit_style,"metal") == 0) {
    ...
    global_e = 1.0;
  } else error->all(FLERR,"Pair tersoff/mod/zbl requires metal or real units");
}

/* ---------------------------------------------------------------------- */

void PairTersoffMODZBL::read_file(char *file)
{
  memory->sfree(params);
  params = nullptr;
  nparams = maxparam = 0;
  ...
  if (comm->me == 0) {
    PotentialFileReader reader(lmp, file, "tersoff/mod/zbl", unit_convert_flag);
    char * line;
...
void PairTersoffMODZBL::repulsive(Param *param, double rsq, double &fforce,
                               int eflag, double &eng)
{
  double r,tmp_fc,tmp_fc_d,tmp_exp;

  // Tersoff/MOD repulsive portion
...
double PairTersoffMODZBL::ters_fa(double r, Param *param)
{
...
double PairTersoffMODZBL::ters_fa_d(double r, Param *param)
{
  if (r > param->bigr + param->bigd) return 0.0;
...
double PairTersoffMODZBL::F_fermi(double r, Param *param)
{
...
double PairTersoffMODZBL::F_fermi_d(double r, Param *param)
{
